...
open.seir.model <- function (t, x, params) {
  ...
  death<-x*params["mu"]
  list(c(-infec+born-death[1],infec-symp-death[2],symp-recov-death[3],recov-death[4]))
  }
...
recover <- params["gamma"]*x[2]
death<-x*params["mu"]
list(c(-infect+born-death[1],infect-recover-death[2],recover-death[3])) # This needs to be 4
}


xstart <- c(S=999,I=1,R=0)
xstart_sier<-c(S=999,I=1,E=0,R=0)
params <- c(B=2E4,mu=1/50,beta=1000,gamma=365/5)
times <- seq(from=0,to=25,by=1/365)
...
out_sier<-melt(out_sier,id='time')
mutate(out_sier,Id="SIER")
out<-rbind(out_sir,out_sier)
ggplot(data=out,mapping=aes(x=time,y=value,color=variable))+geom_line(size=2)+theme_classic()+labs(x='time (yr)',y='number of individuals')+facet_wrap(~Id)
