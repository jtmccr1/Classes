...
###Exercise 3
For the next two exercises I'm using a starting population of 1 million.  This should be the stready state for population allowing us to focus on the popualtion dynamics caused by the outbreak
```{r}
...
I'll make the assumption that the as soon as someone has recovevered, they are immune for ten years.  So the R compartment is the "immune" compartment and after ten years of immunity the inidividual will cycle back into the susceptible compartment.
...
We discussed in classes that the periodic behavior of the infection outbreaks was due to the fact that one outbreak drained the susceptible population until the birth rate built the number of susceptibles up again before another, smaller outbreak hit.  It looks like in the SIRS model the period and depth of these outbreaks is decreased.  This makes sense we are refilling the susceptible population faster.  Interstingly it looks like long-term behavior is splightly different. In the case were immunity is not eternal there is a highler level of infection once the model reaches the steady state.  I suppose this makes sense as in the SIRS we are replenishing the susceptible popualtion faster.
