...
nll.op<-function(b,m,carapace.length,serostatus){
...
params <- expand.grid(b=seq(-0.001,0.01,by=0.0001),
                      m=seq(-0.000001,0.0005,0.00001))
ddply(params,~b+m,mutate,
      loglik=with(gt1,-nll(b,m,carapace.length,serostatus))) -> params
...
ggplot(data=params,mapping=aes(x=m,y=b,z=loglik))+
  ...
  geom_point(x=p1['m'],y=p1["b"],shape=3)+ geom_point(x=p2['m'],y=p2["b"],shape=3)+
