...
> In looking at the Ricker equation for density dependent population growth $N_t=N_{t-1}\exp{r(1-\frac{N_{t-1}}{K})}$  We can see that $N_t<N_{t-1}$ only when $r(1-\frac{N_{t-1}}{K})<0$ which is certainly not required by $r<1$  So $r<1$ by no means requires negative growth

I don't know much about bird ecology but 200 birds as a carrying capacity seems quite small.
***Not a big forest....***

> In looking at the data there are cases where the population grows above 200 but these are almost always short lived and followed by a decrease in the population.  On average the number of birds seems to flucuate between 50 and 175 so 200 isn't that bad of an estimate.
...
> Using confit we can see that as the level of confidence decreases the area covered by the confindence interval also decreases.  Which makes sense since as larger p level must include the true parameter more often. I'm looking just at the intercept here for ease of plotting
...
> As mentioned above, there are cases where the population exceeds 200 birds; however, these cases are almost always followed by a decline of the population.  So while 200 can't be the carrying capacity, since it is exceeded, it probably isn't far off.
...
> Again I was readying theses tests backwards.  We can not reject the null hypothesis in favor of the alternative suggesting that autocorrelation is greater than 0.
...
> If we look at months as a factor the model does marginally better although there is still not enought evidence to suggest homoskedasticity and we reject the hypothesis that the residuals are normally distributed. 
